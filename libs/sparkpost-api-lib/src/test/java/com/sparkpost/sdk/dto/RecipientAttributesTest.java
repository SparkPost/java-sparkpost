...
public class RecipientAttributesTest {
	...
	public void testDecodeRecipientAttributes() {
		Gson gson = new Gson();
		RecipientAttributes recipientAttributes = gson.fromJson(RECIPIENT_ATTRIBUTES_JSON, RecipientAttributes.class);
		...
		Assert.assertTrue(tags.containsAll(expected));
		 ...
		 assertThat(metadata, hasEntry("place", "Bedrock"));  
		 
		 Map<String, String> substitutionData = recipientAttributes.getSubstitutionData();
		 assertThat(substitutionData, hasEntry("customer_type", "Platinum"));  
	}
