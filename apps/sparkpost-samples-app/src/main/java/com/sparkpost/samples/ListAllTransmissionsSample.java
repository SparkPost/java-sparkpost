...
/**
 * List an array of transmission summary objects. A transmission summary object contains 
 * the "template_id", "id", "campaign_id", "description", and "state". The following lists 
 * are supported:
 * <ul>
 ...
 *     <li>Multi-recipient transmissions for a campaign that use a specific template</li>
 * </ul>
 * 
 * Note that single recipient transmissions are not returned.
 * 
 * By default, the list includes all transmissions for all campaigns. Use the template_id 
 * parameter to specify a template and the campaign_id parameter to specify a campaign.
 * The response for transmissions using an inline template will include "template_id":"inline". 
 * Inline templates cannot be specifically queried.The example response shows a query on 
 * campaign_id=thanksgiving, with template_id not specified as part of the query. 
 *
 */
public class ListAllTransmissionsSample extends SparkPostBaseApp {
		...
		private void listAllTransmissions() throws SparkPostException {
